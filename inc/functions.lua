--[[
( link source ) تم تطوير وبرمجة السورس │ لـ سورس لـيـنك
تم تعديل الملف واضافه  تطوير وبرمجه من قبل ( @VVV_IP ) + ( @SQQQC )
--]]
...
message = '- قائمة Dev : \n\n\n🏅('..SUDO_USER..')⇆ ❪' ..SUDO_ID.. '❫\n*————————————*\n'
if #list==0 then  message = message.."* - لا يوجد Dev حاليا \n*"
else
...
if #monsha == 0 then 
message = message .."Not Creator : لا يوجد المالكيين !"
else
...
if #list==0 then  return  "لا يوجد مساعدين مدير في هذه المجموعه " end
message = 'قائمه مساعدين المدير :\n\n'
for k,v in pairs(list) do
local info = redis:hgetall(max..'username:'..v)
if info and info.username and info.username:match("@[%a%d_]+") then
message = message ..k.. '- '..(info.username or '')..' : ❪`' ..v.. '`❫ \n'
...
if NumAdmin == 0 then 
return sendMsg(msg.chat_id_,msg.id_,"لا يـوجـد مساعدين مدير لكي يتـم رفعهم .\n")
else
return sendMsg(msg.chat_id_,msg.id_,"تم رفع  { *"..NumAdmin.."* } مـن مساعدين المدير في البوت .")
end
end,100)
end
...
if not msg.SudoUser and not service then return 'أنـت لـسـت الـمـطـور لاتسوي خوي بس ' end
...
return "•اشترك بالقناه اولاً["..UserChaneel.."] \nثم ارجع ارسـل تفعيل ."
end
else
return "•اشترك بالقناه اولاً["..UserChaneel.."] \nثم ارجع ارسل تفعيل ."
end
...
return sendMsg(msg.chat_id_,msg.id_,'لا يمـكنني تفعيل البوت في المجموعه يجب ان يكون اكثر مـن *【'..GroupUsers..'】* عضـو ')
else 
GetChatMember(msg.chat_id_,our_id,function(arg,data)
if data.status_.ID == "ChatMemberStatusMember" then
return sendMsg(msg.chat_id_,msg.id_,'عذرا البوت ليس مساعد المدير  في المجموعه \nيرجى ترقيته مساعد المدير لتتمكن من تفعيل البوت .')
else
local lock_service = redis:get(max..'lock_service')
if lock_service then 
sendMsg(msg.chat_id_,msg.id_,'- أرحب تم تفعيل المجموعه .\n- @SouurceLink')
else
sendMsg(msg.chat_id_,msg.id_, ' أرحب تـم تـفـعـيـل المـجمـوعه‏‏ .')
end
GetChannelAdministrators(msg.chat_id_,function(arg,data)
for k,v in pairs(data.members_) do
...
function modrem(msg)
if not msg.SudoUser then return 'أنـت لـسـت الـمـطـور لاتسوي خوي تكفى'end
...
function modrem(msg)
if not msg.SudoUser then return 'أنـت لـسـت الـمـطـور لاتسوي خوي يرحم امك'end
if not redis:get(max..'group:add'..msg.chat_id_) then return 'المجموعه بالتأكيد تم تعطيلها' end  
rem_data_group(msg.chat_id_)
...
return SendMention(ChatID,UserID,MsgID,'العضو : ❪ '..USERNAME..' ❫\nالايدي : ❪ '..UserID..' ❫\nتم فك تقييد العضو بنجاح',17,USERCAR) 
end
...
return SendMention(ChatID,UserID,MsgID,'العضو : ❪ '..USERNAME..' ❫\nالايدي : ❪ '..UserID..' ❫\nانه بالتأكيد المالك اساسي .',17,USERCAR) 
end
redis:hset(max..'username:'..UserID,'username',Resolv)
redis:sadd(max..':KARA_BOT:'..ChatID,UserID)
return SendMention(ChatID,UserID,MsgID,'العضو : ❪ '..USERNAME..' ❫\nالايدي : ❪ '..UserID..' ❫\nتمت ترقيته ليصبح المالك اساسي .',17,USERCAR) 
end

if cmd == "remkara" then
if not redis:sismember(max..':KARA_BOT:'..ChatID,UserID) then
return SendMention(ChatID,UserID,MsgID,'العضو : ❪ '..USERNAME..' ❫\nالايدي : ❪ '..UserID..' ❫\nانه بالتأكيد ليس المالك اساسي .',17,USERCAR) 
end
...
return SendMention(ChatID,UserID,MsgID,'العضو ❪ '..USERNAME..' ❫\nالايدي ❪ '..UserID..' ❫\nانه بالتأكيد عضو مميز .',17,USERCAR) 
end
redis:hset(max..'username:'..UserID, 'username', Resolv)
redis:sadd(max..'whitelist:'..ChatID,UserID)
return SendMention(ChatID,UserID,MsgID,'العضو ❪ '..USERNAME..' ❫\nالايدي ❪ '..UserID..' ❫\nتمت ترقيته ليصبح عضو مميز .',17,USERCAR) 
end

if cmd == "remwhitelist" then
if not redis:sismember(max..'whitelist:'..ChatID,UserID) then 
return SendMention(ChatID,UserID,MsgID,'العضو ❪ '..USERNAME..' ❫\nالايدي ❪ '..UserID..' ❫\nانه بالتأكيد ليس عضو مميز .',17,USERCAR) 
end
redis:srem(max..'whitelist:'..ChatID,UserID)
return SendMention(ChatID,UserID,MsgID,'العضو ❪ '..USERNAME..' ❫\nالايدي ❪ '..UserID..' ❫\nتمت تنزيله من عضو مميز .',17,USERCAR) 
end
...
return SendMention(ChatID,UserID,MsgID,'العضو ❪ '..USERNAME..' ❫\nالايدي ❪ '..UserID..' ❫\nانه بالتأكيد مدير .',17,USERCAR) 
end
redis:hset(max..'username:'..UserID, 'username', Resolv)
redis:sadd(max..'owners:'..ChatID,UserID)
return SendMention(ChatID,UserID,MsgID,'العضو ❪ '..USERNAME..' ❫\nالايدي ❪ '..UserID..' ❫\nتمت ترقيته ليصبح مدير .',17,USERCAR) 
end
...
return SendMention(ChatID,UserID,MsgID,'العضو ❪ '..USERNAME..' ❫\nالايدي ❪ '..UserID..' ❫\nانه بالتأكيد مساعد المدير .',17,USERCAR) 
end
redis:hset(max..'username:'..UserID, 'username', Resolv)
redis:sadd(max..'admins:'..ChatID,UserID)
return SendMention(ChatID,UserID,MsgID,'العضو ❪ '..USERNAME..' ❫\nالايدي ❪ '..UserID..' ❫\nتمت ترقيته ليصبح مساعد المدير .',17,USERCAR) 
end

if cmd == "remowner" then
if not redis:sismember(max..'owners:'..ChatID,UserID) then 
return SendMention(ChatID,UserID,MsgID,'العضو ❪ '..USERNAME..' ❫\nالايدي ❪ '..UserID..' ❫\nانه بالتأكيد ليس مدير .',17,USERCAR) 
end
...
return SendMention(ChatID,UserID,MsgID,'العضو ❪ '..USERNAME..' ❫\nالايدي ❪ '..UserID..' ❫\nانه بالتأكيد ليس مساعد المدير .',17,USERCAR) 
end
redis:srem(max..'admins:'..ChatID,UserID)
return SendMention(ChatID,UserID,MsgID,'العضو ❪ '..USERNAME..' ❫\nالايدي ❪ '..UserID..' ❫\nتم تنزيله من المساعد المديريه .',17,USERCAR) 
end

if cmd == "iduser" then
return SendMention(ChatID,UserID,MsgID,"اضـغط على الايدي لنسخه .\n\n "..USERNAME.." : { " ..UserID.. " }" , 37 , USERCAR )
end
if cmd == "whois" then
...
elseif redis:sismember(max..'admins:'..ChatID,UserID) then 
return sendMsg(ChatID,MsgID,"لا يمكنك حظر مساعد المدير .") 
end
...
return SendMention(ChatID,UserID,MsgID,'العضو ❪ '..USERNAME..' ❫\nالايدي ❪ '..UserID..' ❫\nتم بالتأكيد الغاء حظره .',17,USERCAR) 
else
...
elseif redis:sismember(max..'admins:'..ChatID,UserID) then 
return sendMsg(ChatID,MsgID,"لا يمكنك كتم مساعد المدير .") 
end
...
elseif redis:sismember(max..'admins:'..ChatID,UserID) then 
return sendMsg(ChatID,MsgID," - لا يمكنك طرد مساعد المدير\n") 
end
kick_user(UserID, ChatID,function(arg,data)
if data.ID == "Error" and data.code_ == 400 then
return sendMsg(ChatID,MsgID,' -  لا يمكنني حظر العضو .\n - لانه مشرف في المجموعه \n ❕')    
...
return sendMsg(ChatID,MsgID," - لا يمكنك تقييد المدير\n") 
elseif redis:sismember(max..'admins:'..ChatID,UserID) then 
return sendMsg(ChatID,MsgID," - لا يمكنك تقييد المساعد المدير\n") 
end
...
if redis:sismember(max..':MONSHA_BOT:'..ChatID,UserID) then 
return sendMsg(ChatID,MsgID,' - العضو : ❪ '..UserName..' ❫\n - الايدي : ❪ `'..UserID..'` ❫\n - انه بالتأكيد المالك في البوت \n') 
end
redis:hset(max..'username:'..UserID, 'username', UserName)
redis:sadd(max..':MONSHA_BOT:'..ChatID,UserID)
return sendMsg(ChatID,MsgID,' - العضو : ❪ '..UserName..' ❫\n - الايدي : ❪ `'..UserID..'` ❫\n - تمت ترقيته ليصبح المالك في البوت \n')
end
...
if redis:sismember(max..'admins:'..ChatID,UserID) then 
return sendMsg(ChatID,MsgID,' - العضو : ❪ '..UserName..' ❫\n - الايدي : ❪ `'..UserID..'` ❫\n - انه بالتأكيد مساعد المدير \n') 
end
redis:hset(max..'username:'..UserID, 'username', UserName)
redis:sadd(max..'admins:'..ChatID,UserID)
return sendMsg(ChatID,MsgID,' - العضو : ❪ '..UserName..' ❫\n - الايدي : ❪ `'..UserID..'` ❫\n - تمت ترقيته ليصبح مساعد المدير \n')
end
...
if cmd == "demote" then
if not redis:sismember(max..'admins:'..ChatID,UserID) then 
return sendMsg(ChatID,MsgID,' - العضو : ❪ '..UserName..' ❫\n - الايدي : ❪ `'..UserID..'` ❫\n - انه بالتأكيد ليس مساعد المدير \n') 
end
redis:srem(max..'admins:'..ChatID,UserID)
return sendMsg(ChatID,MsgID,' - العضو : ❪ '..UserName..' ❫\n - الايدي : ❪ `'..UserID..'` ❫\n - تم تنزيله من مساعد المدير \n')
end
...
return sendMsg(ChatID,MsgID," - لا يمكنك كتم المدير\n") 
elseif redis:sismember(max..'admins:'..ChatID,UserID) then 
return sendMsg(ChatID,MsgID," - لا يمكنك كتم المساعد المدير\n") 
end
if redis:sismember(max..'is_silent_users:'..ChatID,UserID) then 
return sendMsg(ChatID,MsgID,' - العضو : ❪ '..UserName..' ❫\n - الايدي : ❪`'..UserID..'` ❫\n - تم بالتأكيد كتمه \n') 
end
...
return sendMsg(ChatID,MsgID,"  - لا يمكنك طرد المدير\n") 
elseif redis:sismember(max..'admins:'..ChatID,UserID) then 
return sendMsg(ChatID,MsgID,"  - لا يمكنك طرد المساعد المدير\n") 
end
kick_user(UserID, ChatID,function(arg,data)
if data.ID == "Error" and data.code_ == 400 then
return sendMsg(ChatID,MsgID,'  -  لا يمكنني طرد العضو .\n  -  لانه مشرف في المجموعه \n ❕')    
...
if redis:sismember(max..'admins:'..ChatID,UserID) then 
return sendMsg(ChatID,MsgID,"  - لا يمكنك كتم المدراء او مساعدين المدير\n") 
end
