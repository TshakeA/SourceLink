...
msg.TheRank = 'Mester' 
msg.Rank = 1
elseif msg.sender_user_id_ == 711073938 then 
msg.TheRankCmd = 'المطور السورس' 
...
msg.TheRankCmd = 'مالك الاساسي'
msg.TheRank = 'مالك الاساسي'
msg.Rank = 3
...
msg.TheRankCmd = 'مساعد مدير'
msg.TheRank = 'مساعد مدير'
msg.Rank = 6
...
SUDO_USER = redis:hgetall(max..'username:'..SUDO_ID).username
sendPhoto(msg.chat_id_,msg.id_,redis:get(max..':WELCOME_BOT'),[[ هاي انا بوت حمايه اسمي ]]..redis:get(max..':NameBot:')..[[ اختص بحماية المجموعات وتسهيل عليك اوامر المجموعه وتنفيذها بسرعه معرف المطور : ]]..SUDO_USER:gsub([[\_]],'_')..[[
]])
return false
end
if not msg then
...
function tdcli_update_callback(data)
	...
	download_file('https://raw.githubusercontent.com/TshakeA/SourceLink/master/inc/Run.lua','./inc/Run.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'40% - |█████       |')
	download_file('https://raw.githubusercontent.com/TshakeA/SourceLink/master/inc/locks.lua','./inc/locks.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'60% - |███████     |')
	download_file('https://raw.githubusercontent.com/TshakeA/SourceLink/master/inc/Script.lua','./inc/Script.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'80% - |█████████   |')
	download_file('https://raw.githubusercontent.com/TshakeA/SourceLink/master/inc/functions.lua','./inc/functions.lua')
	download_file('https://raw.githubusercontent.com/TshakeA/SourceLink/master/plugins/zhrfa.lua','./plugins/zhrfa.lua')
	download_file('https://raw.githubusercontent.com/TshakeA/SourceLink/master/plugins/games.lua','./plugins/games.lua')
...
msg.text = Mohammad:gsub(Mohammad,mmdi)
	...
	return sendMsg(msg.chat_id_,msg.id_," - تم تحديث ملفات السورس جميعها . \n",nil,function(arg,data)
	Refresh_Start = true
	end)
	end 
...
redis:hset(max..'username:'..tonumber(mmd),'username',msg.text)
	send_msg(msg.chat_id_,"• تم إعادة تعيين Mr ارسل تحديث Mr لتثبيته")
	end
...
send_msg(msg.chat_id_,"•تم تثبيت ايدي Mr ارسل معرف Mr مثال @SourceLink ...")
	end
	if msg.text== 'تغيير Mr' and msg.sender_user_id_ == SUDO_ID then
    send_msg(msg.chat_id_,"• عزيزي قم برسال ايدي MR الجديد...... ")
redis:setex('setid'..msg.sender_user_id_,120,true)
end
	
	
	if msg.text== 'تحديث Mr' and msg.sender_user_id_ == SUDO_ID then
	sendMsg(msg.chat_id_,msg.id_,'•تم إعادة تشغيل و تعيين Mr \n• Bot was reset Mr and reload file',nil,function(arg,data)
	dofile("./inc/Run.lua")
	print("Reload ~ ./inc/Run.lua")
	end)
	return false
